    ...
    useEffect(() => {
      const verify = async () => {
        ...
        if (!reference || reference === "PENDING") {
          // KoraPay redirects back with reference in URL — check for kora_reference too
          const koraRef = searchParams.get("kora_reference") || searchParams.get("trxref");
          if (!koraRef) {
            setStatus("failed");
            return;
          }
        ...
        const ref = searchParams.get("reference") || searchParams.get("kora_reference") || searchParams.get("trxref");
